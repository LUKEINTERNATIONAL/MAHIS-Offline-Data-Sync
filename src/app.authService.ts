    ...
    try {
        ...
        if (existingPayload) {
            if (existingPayload.patientID == patient.patientID) {
                ...
                const M_patient = sophisticatedMergePatientData(JSON.parse(existingPayload.data) as any, patient as any) as any;
                existingPayload.data = JSON.stringify(M_patient);
